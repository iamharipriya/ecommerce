...
app.get('/cart', async (req, res) => {
    const pct = await addpctModel.find({})
    return res.render('cart', { pct })
})
//signup page render
...
app.post("/delete/:id", async (req, res) => {
    try {
        await addpctModel.findByIdAndDelete(req.params.id); // Delete the book
        res.redirect("/cart"); // Redirect back to the home page
    } catch (err) {
        res.status(500).send("Error deleting book");
    }
});
const addpctschema = mongoose.Schema({
    pctimg: { type: String, required: true },
    pctname: { type: String, required: true },
    pctdis: { type: String, required: true },
    pctprice: { type: String, required: true }
})
const addpctModel = mongoose.model('hari addpct', addpctschema)

app.get("/addcart/:id", async (req, res) => {
    try {
        const pct = await pctModel.findById(req.params.id);
        const temppct = new addpctModel({
            pctimg: pct.pctimg,
            pctdis: pct.pctname,
            pctname: pct.pctdis,
            pctprice: pct.pctprice

        })
        const datasave = await temppct.save()
        if (datasave) {
            return res.render('addcartok', { message: 'Product Add Successfully!..' })
